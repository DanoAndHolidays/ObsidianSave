...
机械规范化结束后，按 [内容审核与变更标记规则](content-review-rules.md) 审核流畅性、事实正确性、代码、顺序、完整性、去重和术语。每个语义调整必须在修改位置附近使用 `*已修改*`、`*已补充*` 或 `*已纠正*` 局部标记，不能静默修改；不要使用 `CR-xxx` 或文末变更记录。
...
| 局部标记与旧协议残留检查 | 脚本 | `validate_content_review.py` |
