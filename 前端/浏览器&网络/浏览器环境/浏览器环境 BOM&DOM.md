...
![[Pasted image 20260618173738.png]]
...
BOM 是通用 [HTML 规范](https://html.spec.whatwg.org/) 的一部分。在 [https://html.spec.whatwg.org](https://html.spec.whatwg.org/) 中的 HTML 规范不仅是关于“HTML 语言”（标签，特性）的，还涵盖了一堆对象、方法和浏览器特定的 DOM 扩展。这就是“广义的 HTML”。此外，某些部分也有其他的规范，它们被列在 [https://spec.whatwg.org](https://spec.whatwg.org/) 中。
...
|**操作对象**|HTML 文档（标签、文本、属性）|浏览器窗口及浏览器环境|
|**核心入口**|`document`|`window`|
|**标准化**|W3C 标准，跨浏览器一致|无严格标准，依赖浏览器实现|
|**主要功能**|增删改查页面元素、绑定事件|页面跳转、弹窗、定时器、存储、获取屏幕信息|
|**依赖关系**|独立于浏览器（理论上可用于任何 XML）|完全依赖浏览器环境|

1. **包含关系：** `window` 对象包含了 `document` 对象。
    ```javascript
    window.document === document; // true
    ```
    
2. **执行环境：** BOM 提供了运行 JavaScript 的宿主环境，而 DOM 是在这个环境中被操作的具体内容。
3. **协同工作：** 实际开发中经常混用。例如：
    ```javascript
    // 点击按钮（DOM 事件） -> 跳转页面（BOM 行为）
