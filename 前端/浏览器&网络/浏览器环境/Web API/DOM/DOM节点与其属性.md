...
---
## 节点的类
![[Pasted image 20260619203053.png]]类如下所示：
- [EventTarget](https://dom.spec.whatwg.org/#eventtarget) —— 是一切的根“抽象（abstract）”类。
    该类的对象从未被创建。它作为一个基础，以便让所有 DOM 节点都支持所谓的“事件（event）”，我们会在之后学习它。![[Pasted image 20260619203430.png]]
...
节点的全部属性和方法都是继承链的结果。例如，我们考虑一下 `<input>` 元素的 DOM 对象。它属于 [HTMLInputElement](https://html.spec.whatwg.org/multipage/forms.html#htmlinputelement) 类。它获取属性和方法，并将其作为下列类（按继承顺序列出）的叠加：
...
但对于 DOM 元素，它们是不同的：
- `console.log(elem)` 显示元素的 DOM 树。![[Pasted image 20260619203935.png]]
- `console.dir(elem)` 将元素显示为 DOM 对象，非常适合探索其属性。![[Pasted image 20260619203917.png]]
...
- 在 [规范](https://dom.spec.whatwg.org/#node) 中还有一些其他值。![[Pasted image 20260619165459.png]]

![[Pasted image 20260619204805.png]]
...
换句话说，`tagName` 仅受元素节点支持（因为它起源于 `Element` 类），而 `nodeName` 则可以说明其他节点类型。![[Pasted image 20260619205347.png]]
...
标签名称始终是大写的，除非是在 XML 模式下。在 XML 模式中，大小写保持为“原样”。如今，XML 模式很少被使用。
...
`outerHTML` 属性包含了元素的完整 HTML。就像 `innerHTML` 加上元素：
```html
