...
### `DOCTYPE` 很重要
请注意：当 HTML 中没有 `<!DOCTYPE HTML>` 时，顶层级（top-level）几何属性的工作方式可能就会有所不同。可能会出现一些稀奇古怪的情况。在现代 HTML 中，我们始终都应该写 `DOCTYPE`。
...
这些属性是只读的。我们也可以从 `window` 的 `scrollX` 和 `scrollY` 属性中获取滚动信息
...
必须在 DOM 完全构建好之后才能通过 JavaScript 滚动页面。如果我们尝试通过 `<head>` 中的脚本滚动页面，它将无法正常工作。
