...
### 变更型与复制型 API
- *【补充】* `sort`、`reverse`、`splice` 会原地修改数组；状态管理中优先使用 `toSorted`、`toReversed`、`toSpliced` 和 `with`，或在兼容目标不支持时先复制。[API](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/toReversed)对于这些 in place 的函数（原地修改），他们的返回值与原数组一致：
```ts
const arr = [1, 2, 3]
const arr2 = arr.sort()
...
##### splice
`splice()`方法是修改`Array`的“万能方法”，它可以从指定的索引（包括）开始删除若干元素，然后再从该位置添加若干元素：
```javascript
