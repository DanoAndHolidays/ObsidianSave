...
![[Pasted image 20260610223046.png]]

---
## 这个面板解决什么问题
假设你有一个后台管理系统，管理员可以给角色分配权限。在"编辑角色权限"页面上，用户改了一堆勾选，但——

1. **他改了什么？** 他自己可能都忘了最初哪些是勾上的（右侧的diff视图）
...
---
## 单元测试怎么测：把纯逻辑从 UI 中剥离
### 9.1 为什么要把函数提取到 utils.ts
...
所以把这三个函数从组件文件中提出来，放到 `utils.ts`，单独导出：
```text
PermissionDemo/
