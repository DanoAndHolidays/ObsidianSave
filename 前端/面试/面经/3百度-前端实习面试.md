...
- 对于图片的object-position这个属性的常用的三个值
  > ⚠️ 纠错：面试官可能问的是 `object-fit` 而非 `object-position`，`object-fit` 的常用值为 `fill`、`contain`、`cover`；`object-position` 用于设置图片在容器内的位置，常用值为 `center`、`top`、`left` 等。两者是不同的属性。
- 手机的进程与线程的个数
- 数据库事务的四个特征
- 计算机网络，抓包
...
##### 5. object-position 常用值

> ⚠️ 纠错：面试官可能问的是 `object-fit` 的常用值（`fill`、`contain`、`cover`）。以下为 `object-position` 的参考：
