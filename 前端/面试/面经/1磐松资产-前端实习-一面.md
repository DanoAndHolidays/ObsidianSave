...
`reduce` 是数组方法，用于对数组元素逐个处理并累加得到最终结果，核心语法：`arr.reduce((acc, curr, idx, arr) => {}, initialValue)`  
...
将「类数组对象」或「可迭代对象（Iterable）」转为真正的数组，支持映射和绑定this，语法：`Array.from(arrayLike, mapFn?, thisArg?)`  
...
##### 事件循环机制（Event Loop）分析

##### 核心结论

JS 是单线程语言，事件循环是解决「异步任务执行顺序」的核心机制，分为「浏览器环境」和「Node 环境」，前端重点关注浏览器环境。
...
页面背景可点击，点击后将三角形由红色转为粉色（再次点击可切换回红色，可选优化）。  
...
- promise.allsettled和promise.all的区别❌️
  > ⚠️ 纠错：正确写法为 `Promise.allSettled` 和 `Promise.all`（首字母大写）
...
规定时间内只执行一次函数，避免高频事件（滚动、 resize、输入框搜索）触发过多请求/计算。
...
触发事件后延迟n秒执行，若n秒内再次触发则重新计时，避免高频事件重复执行。
...
### 项目业务类问题（结合业务场景+技术选型原因）

##### 一刻短剧"滑动5个就重新刷新"的设计原因
