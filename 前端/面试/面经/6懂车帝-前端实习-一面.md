...
- 浏览器缓存，如果强缓存的状态码是多少
  > ⚠️ 纠错：「转态码」应为「状态码」
- 存储位置，什么什么catch？？
  > ⚠️ 纠错：「catch」可能指的是「Cache」（缓存），面试官可能在问浏览器缓存的存储位置（Memory Cache / Disk Cache / Service Worker Cache）
- 图片的预加载还没有从服务器上拉下来，但是我真实要加载的图片已经发送了请求，那么这时候预加载还有效吗？？？这又是什么，怎么就问这些偏难怪

---
## 复盘反思

其实是很好的一次面试，就算我没有通过，他也给我提供了提升的方向，而不是不知道自己该学习什么。我希望自己能不断的提升自己直到成为一个优秀的前端工程师（当然是高级的前端工程师）
 ...
 * @returns {Promise<any[]>} 按照urls顺序返回的结果数组
 */
function concurRequest(urls, maxCount) {
    ...
    return new Promise((resolve) => {
        ...
        function fetchNext() {
            ...
            while (activeCount < maxCount && currentIndex < len) {
                ...
                // 发起请求
                fetch(urls[i])
                    .then((response) => response.json()) // 假设响应是 JSON 格式
                    .catch((error) => {
                        // 请求失败时，将其作为一个错误对象存入结果数组
                        console.error(`请求 ${urls[i]} 失败:`, error)
                        return { error: true, message: error.message } // 可以自定义错误格式
                    })
                    .then((data) => {
                        // 将结果按原始顺序存入数组
                        results[i] = data
...
> ##### 关键点
> 使用"滑动窗口"思想，通过 `running` 控制并发数，`finally` 中继续调度新任务。

---

### 手写 Vue 数据双向绑定（Vue 2 风格）
...
`Object.defineProperty` 无法监听数组下标赋值（如 `arr[0] = 1`）和长度变化。
...
使用 `Proxy`，天然支持数组索引和长度变化，无需 hack。
...
|模式|全称|核心思想|组件职责|
|---|---|---|---|
|**MVC**|Model-View-Controller|Controller 作为中介，处理用户输入，更新 Model 和 View|- **Model**：数据与业务逻辑 - **View**：UI 展示 - **Controller**：接收输入，协调 M 和 V|
|**MVVM**|Model-View-ViewModel|ViewModel 通过**数据绑定**自动同步 View 和 Model|- **Model**：同上 - **View**：UI，绑定到 ViewModel - **ViewModel**：暴露数据和命令，自动同步（无需手动操作 DOM）|
...
> 预加载图片 A（`new Image().src = A`），但正式请求 A 已发出。此时预加载还有效吗？
...
> ##### 例外
> 若预加载和正式请求的 **headers 不同**（如 CORS、Referer），可能被视为不同资源，无法共享。

##### 结论

预加载在绝大多数情况下有效，是常用的性能优化手段。
