...
🔥为常考，⌚️为我面试的时候考过，❌️为没看或者看了没啥用

[秋染蒹葭 的个人主页 - 动态 - 掘金](https://juejin.cn/user/1468603262570615)
...
## ⌚️大数相加解决（字符串相加）
传两个字符串进来，返回一个字符串。
这里处理很妙，对于长度不一样的字符串，肯定有一个是加到最后会有剩余有的，这里我们直接==用0来填充不够长度的字符串的位==，保证逻辑可以顺利的运行：
```js
...
---
## js下划线转驼峰处理「快手」
正则法，`/_([a-z])/g`表达的意思是在全局情况下，寻找在`_`后面的小写的字符。找到后，replace将match的内容替换为函数返回的值
...
作者：秋染蒹葭
链接：[https://juejin.cn/post/7299357176928354313](https://juejin.cn/post/7299357176928354313)
来源：稀土掘金
