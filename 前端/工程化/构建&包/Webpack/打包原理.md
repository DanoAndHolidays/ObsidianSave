...
[https://juejin.cn/post/6844904146827476999?searchId=2026050921455564643296AEC6761CE681](https://juejin.cn/post/6844904146827476999?searchId=2026050921455564643296AEC6761CE681)
...
---
## 读取配置，确立入口
Webpack 启动时，根据配置文件（`webpack.config.js`）或 CLI 参数，找到**入口文件**（entry）。入口告诉 Webpack 从哪个模块开始构建依赖关系（比如 `./src/index.js`）。
...
最终生成的代码核心是一个**模块定义对象**，以及一个 `__webpack_require__` 函数。运行时根据请求的模块 ID，从对象中取出模块的执行结果并返回。
