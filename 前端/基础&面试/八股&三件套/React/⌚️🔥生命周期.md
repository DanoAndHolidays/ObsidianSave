...
Render 阶段可能被中断、重新执行甚至丢弃，因此应该保持纯净，不能随意执行请求、订阅、DOM 操作等副作用。
...
作用：
```text
根据 props + state 计算 UI
```

属于 Render Phase，要求尽量保持纯函数：
```text
不要请求接口
不要操作 DOM
不要产生副作用
...
典型场景：
```text
聊天列表更新前记录 scrollHeight
↓
DOM 插入新消息
↓
componentDidUpdate 根据旧数据恢复位置
```
...
但注意 useEffect 不能简单理解成 componentDidMount / Update / Unmount 的一一对应。
...
```text
默认 useEffect
只有必须在浏览器绘制前处理布局时才使用 useLayoutEffect
```
...
### StrictMode 与生命周期
React 开发环境 StrictMode 下，Effect 可能出现：
```text
setup
↓
cleanup
...
可以回答：
React 的生命周期传统上可以分为挂载、更新和卸载三个阶段。

类组件在挂载阶段主要包括 `constructor`、`getDerivedStateFromProps`、`render` 和 `componentDidMount`；更新阶段包括 `getDerivedStateFromProps`、`shouldComponentUpdate`、`render`、`getSnapshotBeforeUpdate` 和 `componentDidUpdate`；卸载阶段主要是 `componentWillUnmount`。

如果从 React 内部机制理解，生命周期又可以分成 Render Phase 和 Commit Phase。Render 阶段负责根据 props 和 state 计算新的 UI，这个阶段可能被中断或者重新执行，所以应该保持纯净；Commit 阶段才真正修改 DOM，并执行和副作用相关的逻辑。

现代 React 主要使用函数组件，因此实际开发中更多通过 `useEffect` 和 `useLayoutEffect` 管理生命周期相关的副作用。Effect 更准确的理解不是简单对应 `componentDidMount`、`componentDidUpdate` 和 `componentWillUnmount`，而是一个 `setup → cleanup → setup` 的同步过程。

### 函数组件有没有生命周期？
有生命周期过程，但没有类组件那套生命周期方法。主要通过：
```text
useEffect
useLayoutEffect
```
...
### useEffect 等于 componentDidMount 吗？
不等于。`useEffect(..., [])` 在使用场景上类似 `componentDidMount`，但 Effect 的核心模型是：
```text
setup / cleanup
```
...
如果发送请求，会造成副作用重复执行。因此 render 应尽量保持纯净。
...
涉及 DOM 测量和同步布局使用 `useLayoutEffect`，普通副作用优先 `useEffect`。

### 为什么 componentWillMount 等 API 被废弃？
因为这些生命周期位于 Render Phase。在并发渲染下，Render Phase 可能被重复执行、中断或丢弃，副作用放在这里不安全。因此它们后来被标记为：
```text
UNSAFE_
```
