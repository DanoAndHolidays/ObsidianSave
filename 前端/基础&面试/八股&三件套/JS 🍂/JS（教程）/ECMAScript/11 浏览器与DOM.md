...
> [!important] 学习优先级：P0
> - 掌握 DOM 查询、创建、更新、删除和事件委托；批量更新时关注重排、重绘与可访问性。
> - 理解 `window`、`location`、`history`、`document` 等浏览器对象的职责与安全边界。
> - 网络请求优先使用 Fetch 等现代 API，并正确处理 HTTP 状态、CORS、取消、超时和响应解析。
> - 表单处理要兼顾原生校验、受控状态、提交幂等、错误反馈和键盘可用性。
...
> [!warning] 下方保留 XHR、JSONP、Flash 的历史上下文，过时结论已就地标成 *【修正】*；现代浏览器请求以 Fetch/CORS 为主。

### DOM 与事件
- *【补充】* 事件传播分捕获、目标、冒泡阶段；列表和动态节点优先用事件委托，并结合 `closest` 与容器边界判断目标。
- *【补充】* `preventDefault` 阻止默认行为，`stopPropagation` 阻止传播，二者不是一回事；被动监听器中不能阻止默认滚动。
- *【补充】* 使用 `once`、`passive`、`capture` 和 `signal` 明确监听器生命周期；组件销毁时必须释放监听器。详见 [[事件冒泡、捕获与委托 ⌚️]]。
- *【补充】* DOM 读写交错可能造成强制同步布局；批量修改、CSS class 和合适的动画 API 通常更稳妥。

### 网络与浏览器 API
- *【补充】* Fetch 是主流请求入口；始终检查 `response.ok`，理解请求/响应 body 只能消费一次，并用 `AbortSignal` 取消。
- *【修正】* CORS 是基于 HTTP 响应头的跨源访问机制，不应简单称为“HTML5 功能”；JSONP 仅支持 GET 且有脚本注入风险，只用于维护遗留系统。
- *【补充】* URL 解析使用 `URL`/`URLSearchParams`，不要手工拼接查询串。
- *【补充】* 存储、Observer、Worker、Service Worker 等主题应与浏览器环境笔记联读；主线程与 Worker 之间通过结构化克隆/transferable 传递数据。
---
## 旧笔记知识全集（按来源保留）
### 廖雪峰教程 · 7浏览器/7.1DOM/7.1.1 浏览器对象
##### 浏览器对象
...
*8/11/26 【修正】原内容：现代浏览器 AJAX 主要依靠 `XMLHttpRequest`；修改点：新代码通常优先 Fetch，XHR 主要用于遗留兼容或其特定能力。*
在现代浏览器上写AJAX可以使用`XMLHttpRequest`对象，如果不考虑早期浏览器的兼容性问题，现代浏览器还提供了原生支持的Fetch API，以[Promise](https://liaoxuefeng.com/books/javascript/browser/promise/index.html)方式提供。
##### 安全限制
...
*8/11/26 【修正】原内容：将 Flash 列作跨域方案；修改点：Flash 已退出 Web 平台，以下只作为历史背景。*
一是曾经通过Flash插件发送HTTP请求，这种方式可以绕过浏览器的安全限制，但必须安装Flash，并且跟Flash交互。不过Flash用起来麻烦，而且现在用得也越来越少了。
...
*8/11/26 【修正】原内容：将 JSONP 列作常规跨域方案；修改点：JSONP 仅用于遗留系统，只有 GET 且存在脚本注入风险。*
第三种历史方式称为JSONP，它有个限制，只能用GET请求，并且要求返回JavaScript。这种方式跨域实际上是利用了浏览器允许跨域引用JavaScript资源：
...
*8/11/26 【修正】原内容：CORS 是 HTML5 规范；修改点：CORS 是浏览器依据 HTTP 响应头执行的跨源访问机制。*
CORS全称 Cross-Origin Resource Sharing，通过 HTTP 响应头声明跨源访问权限。
