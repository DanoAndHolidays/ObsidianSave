...
2. **`Object.assign(target, ...sources)`**
   将源对象的可枚举属性复制到目标对象，返回目标对象（==浅拷贝==）。这里的source参数越==靠后==会将==靠前==的==覆盖==掉的
...
3. 使用对象字面量创建，这和new出来的是一样的，类似语法糖：
```javascript
// 基础字面量创建
const person = {
  name: "张三", // 属性键值对
...
for (item in Date) {
    console.log('item', Object.getOwnPropertyDescriptor(Date, item))
    // item { value: 'Date', writable: true, enumerable: true, configurable: true }
}
